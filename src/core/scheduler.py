...
def build_scheduler(optimizer: torch.optim.Optimizer, warmup_epochs: int = 5, gamma: float = 0.95):
    """
...
# ---------------------------------------------------------
def build_optimizer(model: torch.nn.Module, base_lr: float = 1e-4, weight_decay: float = 1e-4):
    """
    ...
    for name, param in model.named_parameters():
        ...
        if 'alpha' in name or 'beta' in name or 'redweight' in name or 'blueweight' in name:
    ...
    optimizer = torch.optim.AdamW([
        ...
        {'params': helu_params, 'lr': base_lr, 'weight_decay': 0.0}
    ])
